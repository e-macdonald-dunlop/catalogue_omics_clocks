...
#$ -l h_rt=48:00:00 
#$ -l h_vmem=32G
#$ -o ../logs/a04_clock_500_iterations_log.log
#$ -V
...
for(i in 1:500){
	writeLines("")
	writeLines(paste0("Iteration ",i))
	writeLines("")
	writeLines("")
	heading("Split Into Training & Testing")

	split_data<-split_training_testing(data,i)
	...
	penalised_regression_eddie(training_matrix$predictor_matrix,training_matrix$outcome_matrix,testing_matrix$predictor_matrix,testing_matrix$outcome_matrix,config$outcome,config$method,config$if_elastic_net,iteration=i)

	heading("Number of Variables Selected...")
	x<-n_variables_selected(i)
	writeLines(x)

	#want to write output stats for kk plots
	for_kk_plot<-get_stats(paste0("st03_pred_obs_resid_testing_",i,".tsv"),config$cohort,config$omic_assay)
	head(for_kk_plot)
	write.table(for_kk_plot,paste0("st03_obs_pred_outcome_correlation_stats_",config$cohort,"_",i,".tsv"),col.names=T,row.names=F,quote=F,sep="\t")
	#

	slope_stats<-slope_extract(paste0("st03_pred_obs_resid_testing_",i,".tsv"),config$cohort,config$omic_assay)
	head(slope_stats)
	write.table(slope_stats,paste0("st03_obs_pred_outcome_fit_stats_",config$cohort,"_",i,".tsv"),col.names=T,row.names=F,quote=F,sep="\t")
